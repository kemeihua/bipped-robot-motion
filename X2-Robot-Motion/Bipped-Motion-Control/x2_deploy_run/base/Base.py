...
def set_joint_mode(command, num_actions, config):
    command.cmd_enable = 2  # joint control mode
    for idx in range(num_actions):
        command.kp[idx] = config.robot_config.kpStand[idx]
        command.kd[idx] = config.robot_config.kdStand[idx]
        command.max_torque[idx] = config.robot_config.tau_limit[idx]
